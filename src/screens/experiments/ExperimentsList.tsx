...
const ExperimentsList = function () {
    ...
    const createHandleClickExperiment = function (experimentId: string) {
        return async () => {
            await dispatch(subscribeToExperiment(experimentId));
            history.push(`/main/box/${type}/${experimentId}`);
        };
    };
                    ...
                    >
                        <Stack spacing={1}>
                            <Link
                                ...
                                textColor="inherit"
                                underline="none"
                                onClick={createHandleClickExperiment(experiment.id)}
                            >
                                {experiment.name}
                            </Link>
