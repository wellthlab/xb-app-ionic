...
export const newPasswordSchema = Yup.object().shape({
    password: Yup.string()
        ...
        .min(
            6,
            ({ min }) => Strings.password_must_have_at_least + ' ' + min + ' ' + Strings.password_must_have_at_least_2,
        )
        .max(
            128,
            ({ max }) => Strings.password_can_only_have_at + ' ' + max + ' ' + Strings.password_can_only_have_at_2,
        ),
