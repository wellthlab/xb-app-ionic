...
# PREP SLEEP 1
exp=experiment_creator('SLEEP week 1: PREP',box['prep'], boxweek=0)
exp.add_block(content='Your plan of when and how you will get the earliest outside morning light possible for 15-20 minutes in a row- each day may be a little different of course pending your schedules.')
exp.add_block(content='There is a cumulative effect of getting this outdoor daylight dose: It\'s worth planning or thinking this week ahead so you can get your five days in a row - to have that synchronizing build up.')
exp.add_task('OUTSIDE is KEY')
exp.add_block(content='it really does need to be with your head out of doors - different kinds of windows block different parts of light, as well as affecting the intensity of that light.')
exp.add_task('HAVING A PLAN')
exp.add_block(content='Especially if a deliberate light dose is a new activity for you, it\'s worthwhile to think about how you can make that happen in your day. Can you take 15 minutes for yourself with a tea or coffee - even it it\'s chilly or wet, or to have a wee walk around the block - to get that light. OR take your reading outside for that time.')
exp.add_block(content='If you can\'t get this time just for yourself, how might you make it part of your morning start?')
exp.add_task('FEEL BETTER TOGETHER: SHARING')
exp.add_block(content='is there someone you might invite to share any of these light dose occasions with you? Even if you\'re not together physically, you might agree to virtually do that morning walk, or coffee - just for you.')
prepsleep1=exp.get_id()
exp.upload()

# PREP SLEEP 3
exp=experiment_creator('SLEEP week 3: PREP',box['prep'], boxweek=0)
exp.add_task('HAVING A PLAN')
exp.add_block(content='What do you need to do for this week to have that time to test out feeling better with this sleep time?')
exp.add_block(content='If you already get two hours of sleep before midnight that\'s fantastic. You can see if stacking your DayLight and Darkness, with some of the other checks above,  makes a difference to the quality of that sleep time.')
prepsleep3=exp.get_id()
exp.upload()
...
# SLEEP 3
exp=experiment_creator('Sleep Week 3: Hours Before Midnight',box['sleep'], boxweek=2, nextExperiment=sleep4, prepExperiment=prepsleep3)
exp.add_block(content='This week\'s experiment asks you for just five days to get to sleep about 2 hours before midnight. This one we\'ve been building up to. It can require some planning in advance with work schedules, or if not constrained by that to try something that may seem unnatural  - for some.')
exp.add_block(content='For those who feel like they are post midnight sleepers - we get it. You may be interested in a study that\'s been repeated. A researcher took members of his lab out camping for a weekend - all identifying as folks who get to sleep after midnight. Guess what happened? away from clocks they went to sleep after sundown and got up in the morning - and they stayed re-set this way into the week when they got back to town.')
exp.add_block(content='What this research seems to suggest is that when we let ourselves align with the daylight/night cycle, that\'s what we do.')
exp.add_block(content='Your assessment is - if you give yourself a week to do this practice, how do you feel in the morning? Most folks report that the effects are positive.')
exp.add_expandable_description_block('The science of Hours Before Midnight')
exp.add_subblock(content='Our sleep has cycles during the night. Usually about 5 are needed for full restorative sleep. Something that is part of that first part of that first cycle is a type of sleep - slow wave - that is particularly necessary and restorative - from tissue repair to memory building (learning).')
exp.add_subblock(content='What is amazing is even if we go to sleep later and sleep for the equivalent time of 5 cycles, we miss that first and deepest cylce of recovery, repair, remembering - it\'s just gone.  So by working with hours before midnight you\'re helping your body heal, repair and rebuild better. You put all that effort into work, into learning, into remembering - not getting that first deep cycle is a loss.')
exp.add_step('If you do caffeine, stop by 2pm this week if not before noon if you can handle that for a few days')
exp.add_step('Keep your room dark and quite ')
exp.add_step('Avoid liquids 10 hours after you get up')
exp.add_step('Getting up in the morning if you generally wake in the late morning')
exp.add_step('Practicing your darkness evening doses')
exp.add_step('Practicing your daylight morning doses')
exp.add_task('Hours Before Midnight')
exp.add_block(typ='time-input',label='What time did you get into Bed')
exp.add_block(typ='time-input',label='What time did you get to sleep?')
exp.add_block(typ='select-input',label='How many times do you think you woke up / got up during the night?',options=['0','1','2','3','4','5+'])
exp.add_block(typ='select-input',label='Did you do a morning light dose?',options=['Yes','No'])
exp.add_block(typ='select-input',label='Did you do evening darkness dose?',options=['Yes','No'])
exp.add_block(typ='select-input',label='Before midnight, were you?',options=['On your own','With Others'])
exp.add_block(typ='select-input',label='Before midnight, were you?',options=['At home','At work', 'Other'])
exp.add_block(typ='text-input',label='If you said "other" - where were you',optional=True)
exp.add_block(typ='select-input',label='Did you get some movement in today to help getting to sleep?',options=['Yes','No'])
exp.add_block(typ='select-input',label='Did you have any difficulty getting to sleep?',options=['Yes, it was challenging','Somewhat difficult', 'No problem','Easier than I thought', 'Do that all the time'])
exp.add_task('Sleep Experience')
exp.add_block(typ='select-input',label='You slept last night:',options=['At home','Away from home'])
...
exp.add_block(typ='select-input',label='How restful was your sleep compared to yesterday?',options=['Less','Somewhat less','The same','A little more','A lot more'])
exp.add_block(typ='select-input',label='How do you feel on waking up compared to yesterday?',options=['A little worse','Worse','Same', 'Better', 'Much better'])
exp.autocomplete(reflection_text='Some questions that might help your reflecting: What were any insights or ah ha\'s from this day\'s hours before midnight? What worked?  What are some of the challenges you need solutions for or have found solutions for? what might you try differently for the next night? did you notice anything different about the day time energy, thinking, emotional state, stress, physical performance? Anything else you\'d like to add about anything at all from your sleep work?',add_context=False)
sleep3=exp.get_id()
exp.upload()


# SLEEP 2
exp=experiment_creator('Sleep Box 2: Dark Matters',box['sleep'], boxweek=1)
exp.add_block(content='While getting light as soon as possible after rising is critical for our sleep quality, so too is getting darkness before sleep, and then keeping things dark during the night. That\'s the experiment for this week: how does exploring a darkness practice before bed and through the night affect not only our sleep quality, but how we feel on waking?')
exp.add_expandable_description_block('Artificial Light - disrupts our wiring')
exp.add_subblock(content='Just as that real daylight is so important for re-synchronizing us with the planet, artificial light at night can disrupt that synchrony. So how do we mitigate that artificial light and still have our lives? That\'s the opportunity this week. How does it feel to explore darkness?')
exp.add_expandable_description_block('Benefits of Darkness: Night Vision')
exp.add_subblock(content='Night vision is the ability to see at night. Our eyes have a whole other system of sensors for night time vision. IF we practice deliberately using that system (known as the rod photoreceptors in our eyes that work at night vs the cone receptors that work in daylight) then we get better at using that system.')
exp.add_block(content='The experiment this week is to explore having the least amount of light necessary for navigating our world as the sun goes down.')

exp.add_step('EYE LIGHT at NIGHT - to understand how well your eyes can function in the dark, do a dark dose test. Sit in a darkened room without any lights on and wait for 15 minutes - it takes 15 minutes for processes in our eyes to shift in ways to adapt to darkness.')
exp.add_step('What can you see after 15 minutes?')
exp.add_step('Have a candle - like a small tea light - ready and a lighter or matches, and after  that check of what you can see with no light, see what happens with just that one tea light?')
exp.add_step('Do you have any battery or electric lights that are just that level of light - not brighter? What if you put a cloth over that light shade to dull it?')
exp.add_step('COMPUTER SCREENS: try turning the brightness down as low as it will go on your electronic devices - what\'s the least amount of brightness you need when dark adapted to use these devices?')
exp.add_step('BEDROOM - what do you need to keep your bedroom / home as dark as you can manage so if you wake up in the night, you\'re not needing to put on a light to see where you are or what you\'re doing?')

exp.add_task('Daily Dark Practice')
exp.add_block(typ='time-input',label='Around what time does it get dark (sun down) where you are right now?')
exp.add_block(typ='select-input',label='Were you able to go dark for the evening?',options=['Yes','No'])
exp.add_block(typ='time-input',label='What time did you start going dark?')
exp.add_block(typ='select-input',label='For your dark time, were you:',options=['at home', 'at work', 'other'])
exp.add_block(typ='text-input',label='If you said "other" - what did you use?', optional=True)
exp.add_block(typ='select-input',label='For your dark time, were you:',options=['on your own','with others'])
exp.add_block(typ='select-input',label='What kinds of illumination did you use?',options=['candles','dimmed lamps','red lights','nothing','other'])
exp.add_block(typ='text-input',label='If you said "other" - what did you use?',optional=True)
exp.add_block(typ='select-input',label='To keep your sleeping space dark, what did you use?',options=['curtains','eye mask','room without windows','no lights','red night light only','other'])
exp.add_block(typ='text-input',label='If you said "other" - what did you use?',optional=True)
exp.add_task('Sleep Experience')
exp.add_block(typ='select-input',label='You slept last night:',options=['At home','Away from home'])
exp.add_block(typ='select-input',label='You slept:',options=['By yourself','With others - human/non-human'])
exp.add_block(typ='select-input',label='While trying to fall asleep, you were:',options=['Doing nothing but falling asleep','Reading from paper with a dim light', 'Reading on screen turned down as low as it goes to be legible','Listening to something','Watching something on a screen - no brightness control'])
exp.add_block(typ='select-input',label='How many times do you recall waking up before it was time to get up:',options=['Once','Twice', 'More than twice'])
exp.add_block(typ='select-input',label='If you get up at night do you have any lights come on for whatever you do?',options=['No','Yes'],optional=True)
exp.add_block(typ='select-input',label='If yes, where?',options=['bathroom','bedroom','kitchen','phone screen'],optional=True)
exp.add_block(typ='select-input',label='Was this an unusal day that caused a change in sleep pattern (someone sick, going on a trip, etc)',options=['No', 'Yes'])
exp.add_block(typ='select-input',label='Your dreams were:',options=['Stressful','Positive','Nothing one way or the other'])
exp.add_block(typ='select-input',label='How restful was your sleep compared to yesterday?',options=['Less','Somewhat less','The same','A little more','A lot more'])
exp.add_block(typ='select-input',label='How do you feel on waking up compared to yesterday?',options=['A little worse','Worse','Same', 'Better', 'Much better'])
exp.add_tip('Types of lighting for Dark Times: \n -Candles known as tea lights can be fine: what\'s the least number you need in your environment?. \n ⁃ red lights - where they are not bright - also good if you need some light getting up at night. That tone will not disrupt your night vision.')
exp.autocomplete(reflection_text='Some questions that might help your reflecting: What were any insights or ah ha\'s from this day\'s deliberate darkness?, How was that time for you, focusing on working with darkness in the evening? What worked? What might you try differently? What are some of the challenges you need solutions for or have found solutions for?',add_context=False)
sleep2=exp.get_id()
exp.upload()

# SLEEP 1

exp=experiment_creator('SLEEP Box 1: A.M. light box',box['sleep'], boxweek=0, prepExperiment=prepsleep1, nextExperiment = sleep2)
exp.add_block(content='Great sleep starts way before we go to sleep. It starts with getting great light - that means daylight - as early as possible in the day.')
exp.add_expandable_description_block('WHY THE LIGHT in the MORNING: SYNC\'ing our BODY CLOCK with the EARTH\'s CLOCK')
exp.add_subblock(content='All of the functions of our body are synchronized together; these synchronies are then synchronized with the planet. That synchrony of us with the earth happens by the quality and timing of light that hits a set of nerves in our eyes - the super chiasmic nuclei. IT doesn\'t matter if we can see or not, our eyes still take in light to these nerves, which in turn does a daily reset of our bodies with the earth.')
exp.add_expandable_description_block('LIGHT LAG- HOW WE FEEL NOT BEING IN SYNC')
exp.add_subblock(content='Have you ever had jet lag? When you fly to a different time zone, and your sleep/eating are out of sync with the location? We have that kind of experience when we don\'t get that morning daylight dose  to resync our systems every day. Why? Our own internal clocks are actually a little bit longer -15-20 minutes depending on sex - than the planet\'s 24 hours. If we do not reset with morning light, over the course of the week that\'s like being one to two time zones out of sync; over several weeks that\'s just creating a bigger disregulation. That regular dose of light in the morning resets us to re-align us with the earth.')
exp.add_block(typ='title',content='BONUS')
exp.add_block(content='Getting additional daylight hits during the day is fantastic - for example: getting a mid day dose to complement the morning dose is great - but the morning dose is the priority')
exp.add_step('This week, the experiment is to set up how you can get 15-20 minutes outside in daylight as soon as possible after getting up. You don\'t have to contort yourself to do this. Just have a think about what the earliest time is you will be able to get your head outside, outdoors. And of course, then, do so. Sitting, walking, playing - doesn\'t matter what you do. Even being in the shade is fine. It\'s still bright in day light. Event the cloudiest days are brighter than any indoor lighting. So the main challenge is to find your path to morning outside light.')
exp.add_step('If it\'s that time of year when it\'s dark out when you get up - it\'s still all about getting that bolus of outdoor daylight as soon as you can and in a way that you won\'t need to wear sunglasses.')
exp.add_task('Daily Light Practice')
exp.add_block(typ='time-input',label='What time did you get up this morning?')
exp.add_block(typ='select-input',label='Was the sun up by then?',options=['Yes','No'])
exp.add_block(typ='select-input',label='Did you get your outdoor am dose?',options=['Yes','No'])
exp.add_block(typ='select-input',label='How long was that dose?',options=['less than 10', '10-15', '15-20', 'more than 20'])
exp.add_block(typ='time-input',label='What time did you get your first dose of daylight?')
exp.add_block(typ='select-input',label='Outside was sunny:',options=['Cloudy','Raining','Snowing'])
exp.add_block(typ='select-input',label='How did you get that dose',options=['sitting outside', 'walking outside'])
exp.add_block(typ='select-input',label='How did you get that dose',options=['by yourself', 'with others'])
exp.add_block(typ='select-input',label='How did you get that dose',options=['home', 'other than home'])
exp.add_task('Sleep experience')
exp.add_block(typ='select-input',label='You slept last night:',options=['At home','Away from home'])
exp.add_block(typ='select-input',label='You slept:',options=['By yourself','With others - human/non-human'])
exp.add_block(typ='select-input',label='While trying to fall asleep, you were:',options=['Doing nothing but falling asleep','Reading from paper with a light', 'Reading on screen','Listening to something','Watching something on a screen'])
exp.add_block(typ='select-input',label='How many times do you recall waking up before it was time to get up:',options=['Once','Twice', 'More than twice'])
exp.add_block(typ='select-input',label='Was this an unusal day that caused a change in sleep pattern (someone sick, going on a trip, etc)',options=['No', 'Yes'])
exp.add_block(typ='select-input',label='Your dreams were:',options=['Stressful','Positive','Nothing one way or the other'])
exp.add_block(typ='select-input',label='How restful was your sleep compared to yesterday?',options=['Less','Somewhat less','The same','A little more','A lot more'])
exp.add_block(typ='select-input',label='How do you feel on waking up compared to yesterday?',options=['A little worse','Worse','Same', 'Better', 'Much better'])
exp.autocomplete(reflection_text='Some questions that might help your reflecting: What were any insights or ah ha\'s from this day\'s light dose?, How was that time for you, just focusing on getting that light What worked?, what might you try differently?',add_context=False)
sleep1=exp.get_id()
exp.upload()
