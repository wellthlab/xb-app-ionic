...
import { IonContent, IonPage, IonSpinner, IonCard, IonCardHeader, IonCardTitle, IonCardSubtitle, IonCardContent, IonDatetime, IonButton } from "@ionic/react";
...
const Leaderboard = function (props) {
    ...
    const loadTeams = async function (dates) {
        ...
        let raw;
        try {
            raw = await props.controllers.client.calculateTeamStats({ all: true, dates });
        }
        catch (error) {
            setErrored(true);
            console.error(error);
            ...
            content = teams.map((team) => (
                <IonCard key={team.id}>
                    <IonCardHeader>
                        ...
                        <IonCardSubtitle>{team.userCount} member{team.userCount > 1 ? 's' : ''}</IonCardSubtitle>
                    </IonCardHeader>
                    <IonCardContent>
                        {team.overallScore}
                    </IonCardContent>
