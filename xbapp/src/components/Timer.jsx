...
function Timer(props) {
  ...
  const [isActive, setIsActive] = useState(localStorage.getItem('countActive') != null ? localStorage.getItem('countActive') === 'true' : false);
  ...
  function toggle() {
    ...
    if (!isActive){ //is about to be turned on 
      if (localStorage.getItem('timerStartedAt') != 'null') {
        if (parseInt(localStorage.getItem('timerStartedAt')) == 0) {
          ...
          localStorage.setItem('timerStartedAt', new Date().getTime() - millisec);
        }
      } else localStorage.setItem('timerStartedAt', new Date().getTime());
      
    } else { //about to be paused
  ...
  function reset() {
    setSeconds(0);
    localStorage.setItem('recordedSeconds', null);
    setMinutes(0);
    localStorage.setItem('recordedMinutes', null);
    setHours(0);
    localStorage.setItem('recordedHours', null);
    setIsActive(false);
    localStorage.setItem('countActive', false);
    localStorage.setItem('timerStartedAt', null);
  }
  ...
  return (
        <div >
          <div className="time">{hours > 9 ? hours : '0' + hours}:{minutes > 9 ? minutes : '0' + minutes}:{seconds > 9 ? seconds : '0' + seconds}</div>
          <div className="row">
